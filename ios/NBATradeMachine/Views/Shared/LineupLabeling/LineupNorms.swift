...
nonisolated enum LineupNorms {
    ...
    nonisolated static func feat(_ player: LineupFeatures?, _ name: String) -> Double? {
        player?.value(name)
    }
    ...
    nonisolated static func pctl(_ player: LineupFeatures?, _ name: String,
    ...
    nonisolated static func z(_ player: LineupFeatures?, _ name: String,
                  _ norms: LeagueNorms) -> Double? {
        guard let value = player?.value(name) else { return nil }
    ...
    private nonisolated static func present(_ values: [Double?]) -> [Double] {
        values.compactMap { $0 }
    }

    /// Mean of the lineup's percentiles for `name` (skipping abstainers).
    nonisolated static func meanPctl(_ players: [LineupFeatures?], _ name: String,
                         _ norms: LeagueNorms) -> Double? {
    ...
    nonisolated static func meanZ(_ players: [LineupFeatures?], _ name: String,
                      _ norms: LeagueNorms) -> Double? {
    ...
    nonisolated static func meanRaw(_ players: [LineupFeatures?], _ name: String) -> Double? {
    ...
    nonisolated static func countAbove(_ players: [LineupFeatures?], _ name: String,
    ...
    /// How many players have percentile(`name`) <= `p`. Abstainers don't count.
    nonisolated static func countBelow(_ players: [LineupFeatures?], _ name: String,
                           _ p: Double, _ norms: LeagueNorms) -> Int {
        players.reduce(0) { acc, pl in
    ...
    nonisolated static func countRawBelow(_ players: [LineupFeatures?], _ name: String,
    ...
    /// How many players have raw `name` >= `threshold`. Missing don't count.
    nonisolated static func countRawAtLeast(_ players: [LineupFeatures?], _ name: String,
                                _ threshold: Double) -> Int {
        players.reduce(0) { acc, pl in
            if let v = feat(pl, name), v >= threshold { return acc + 1 }
            return acc
    ...
    nonisolated static func minPctl(_ players: [LineupFeatures?], _ name: String,
                        _ norms: LeagueNorms) -> Double? {
        present(players.map { pctl($0, name, norms) }).min()
    }

    /// All present percentiles for `name`, descending.
    nonisolated static func sortedPctls(_ players: [LineupFeatures?], _ name: String,
                            _ norms: LeagueNorms) -> [Double] {
        present(players.map { pctl($0, name, norms) }).sorted(by: >)
    }

    /// Second-highest percentile for `name` (nil if < 2 players have it).
    nonisolated static func secondPctl(_ players: [LineupFeatures?], _ name: String,
                           _ norms: LeagueNorms) -> Double? {
    ...
    nonisolated static func loadHHI(_ loads: [Double?]) -> Double? {
    ...
    nonisolated static func avgAge(_ players: [LineupFeatures?]) -> Double? {
        meanRaw(players, "age")
    }
}
